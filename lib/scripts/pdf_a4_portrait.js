...
page.onResourceRequested = function (request) {
  if (options.logOnRequestedResource) {
    system.stdout.write('html-pdf: requesting ' + JSON.stringify(request) + '\n');   
  }
}

page.onResourceError = function (resourceError) {
  const error = {
    message: resourceError.errorString,
    status: resourceError.status,
  }
  exit(JSON.stringify(error));
}
